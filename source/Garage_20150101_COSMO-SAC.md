...
σ<sub>new, i</sub> = 
(
  &sum;<sub>j</sub> σ<sub>j</sub> · (RAD<sub>j</sub><sup>2</sup> REFF<sup>2</sup> / (RAD<sub>j</sub><sup>2</sup> + REFF<sup>2</sup>)) 
  · exp( - Term1<sup>2</sup> / (RAD<sub>j</sub><sup>2</sup> + REFF<sup>2</sup>) )
) / 
(
  &sum;<sub>j</sub> (RAD<sub>j</sub><sup>2</sup> REFF<sup>2</sup> / (RAD<sub>j</sub><sup>2</sup> + REFF<sup>2</sup>)) 
  · exp( - Term1<sup>2</sup> / (RAD<sub>j</sub><sup>2</sup> + REFF<sup>2</sup>) )
)
...
2. **Interaction Energy (Δw)**      
The exchange energy between two segments $i$ and $j$ is calculated as:

Δw<sub>ij</sub> = 
(α'/2) · (σ<sub>i</sub> + σ<sub>j</sub>)<sup>2</sup> 
+ c<sub>hb</sub> · max(0, σ<sub>acc</sub> - σ<sub>hb</sub>) 
...
3. **Segment Activity Coefficients (Γs)**     
...
4. **Total Activity Coefficient (γ)**     
...
| [`Binary.m`](Garage_20150101_COSMO-SAC_Binary.m) | Primary function calculating mole fractions, pure/mixture segment gammas, and final $\gamma$. |
...
| [`paraCOSMO.m`](Garage_20150101_COSMO-SAC_paraCOSMO.m) | Provides universal constants ($R$, $c_{hb}$, $\sigma_{hb}$, $\alpha$, etc.). |
